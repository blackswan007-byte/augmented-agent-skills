...
jobs:  
  ...
  suites:  
    ...
    steps:  
      ...
      \#     python tests/run\_all.py \--isolated  
      \- name: Select standard-library-only skills  
        ...
        run: |  
          ...
          manifest \= tomllib.loads(  
              pathlib.Path("tests/skill-requirements.toml").read\_text()  
          )  
          names \= sorted(  
              name  
              for name, entry in manifest\["skills"\].items()  
              if not entry.get("packages") and "python" not in entry  
              and (pathlib.Path("tests") / name).is\_dir()  
          )  
          print(" ".join(names))  
          PY  
          )  
          echo "Selected: $SKILLS"  
          echo "skills=$SKILLS" \>\> "$GITHUB\_OUTPUT"

      \- name: Run suites, one environment each  
        run: uv run \--python 3.13 python tests/run\_all.py \--isolated ${{ steps.select.outputs.skills }}
