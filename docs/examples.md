...
\- Query: "\[scaffold\_name\] AND EGFR AND inhibitor"  
...
\- Calculate QC metrics: n\_genes, n\_counts, pct\_mitochondrial  
\- Identify mitochondrial genes (MT- prefix)  
\- Filter cells: 200 \< n\_genes \< 5000, pct\_mt \< 20%  
...
\- Do not routinely regress out n\_counts or pct\_mt. Current scanpy guidance advises  
...
\- Compute the neighborhood graph on the scVI latent space (n\_neighbors=15)  
...
crossover.  
Criteria: fix the exposure metric, the BLQ rule, and the lambda\_z window  
...
\- python3 allometry\_and\_fih.py \--fih \--noael rat=50,dog=10 \--safety-factor 10  
...
\- Predict the profile with simulate\_regimen.py from the scaled parameters and  
  check that the planned times bracket Tmax and span at least two half-lives  
\- Fix the bioanalytical LLOQ and the BLQ rule now — it changes lambda\_z, and  
...
\- Read the findings, not just the table. Above 20% extrapolated AUCinf, the  
  number is driven by the lambda\_z fit rather than by data; a terminal window  
...
\- python3 fit\_compartmental.py \-i pooled.csv \--dose 500 \--route iv-bolus \\  
...
\- python3 check\_popk\_dataset.py \-i nmdata.csv \--covariates WT,CRCL,ALB \\  
    \--time-varying WT  
...
\- python3 exposure\_response.py \--emax \-i er.csv \--sigmoid  
\- Read fraction\_of\_emax\_reached. If the highest observed exposure reaches a third  
...
\- python3 exposure\_response.py \--cqtc \-i qt.csv \--cmax 250 evaluates the upper  
...
\- python3 bioequivalence.py \-i tablet\_vs\_solution.csv \--design 2x2 \--metric AUC  
...
\- python3 ddi\_static.py \--basic \--ki 0.5 \--imax 2.0 \--fu 0.05 \--dose 0.4, then  
...
\- python3 simulate\_regimen.py \--cl 5 \--v 40 \--dose 500 \--interval 12 \\  
...
\- Fill assets/nca-reporting-checklist.md; an exposure number is uninterpretable  
  without the method, BLQ rule, and lambda\_z window that produced it  
...
\- Route to the clinical pharmacologist, pharmacometrician, and sponsor team. The  
  ...
  monitoring (tdm\_bayes.py) is a separate clinical setting where any regimen  
...
\- NCA table with the method, BLQ rule, and lambda\_z window stated, plus every  
...
\- Filter at E\_hull \< 50 meV/atom, and describe what that means accurately:  
  E\_hull \= 0 is on the hull; a nonzero value is metastable. The 50 meV/atom line is  
...
queried symbol must be in model.gene\_names.  
...
\- Query \`model.gene\_names\` and intersect it with your genes of interest; an  
...
\- Keep the ordered gene list beside the output — \`predict\_genes\` returns values  
  aligned to the list you passed, and an unlabelled column is unusable

Step 4: Run batched inference  
\- Stack processed tiles into batches and call \`predict\_genes\` once per batch with  
...
\- \`check.py facts\` must report \`is\_valid: true\`. Broken geometry gets fixed in the  
...
Expected Output:  
\- Per-animal RELSA trajectories with per-variable weights and n\_vars per time point  
...
\- Run reporting\_lag.py for the pathogen and country in question  
...
\- Run lineage\_prevalence.py with \--top N and no lineage names. It discovers the  
...
\- Run resolve\_lineage.py on the candidate list  
...
\- Run lineage\_prevalence.py for the resolved lineages over the trusted window  
...
\- Run mutation\_profile.py restricted to the gene your assay targets  
...
\- Measure cross-sections:  
  \* σ \= N\_signal / (ε × L × BR)  
  \* N\_signal: number of signal events  
...
\- Write conservation equations:  
  \* Mass balance: dC/dt \= (F\_in × C\_in \- F\_out × C)/V \+ r  
  \* Energy balance: ρCp × dT/dt \= Q \- ΔH\_rxn × r × V  
...
\- Reparameterize Arrhenius around a reference temperature —  
  k \= k\_ref · exp\[−(Ea/R)(1/T − 1/T\_ref)\] — before sampling. In the raw form ln A  
...
\- Optimize UMAP parameters (n\_neighbors, min\_dist)  
...
\- Pin at least one endpoint, give both nodes Biolink categories, and state the  
  ...
  activity\_or\_abundance decreased is a different claim from increased, and an  
...
\- Reference URLs take the form  
  https://paperclip.gxl.ai/citations/{papers|fda|trials}/\<doc\_id\>\#L45, with ranges  
...
\- python3 plan\_validation.py \--framework ich-q2r2 \--attribute impurity \\  
...
\- python3 plan\_validation.py \--framework ich-q2r2 \--attribute impurity \--protocol  
...
\- python3 check\_response.py \-i calibration.csv \--max-back-calc-error 5 \\  
    \--weight 1/x  
...
\- python3 check\_accuracy\_precision.py \-i ap.csv \--accuracy-limit 10 \\  
...
\- python3 check\_detection\_limits.py \--calibration lowrange.csv \--blanks blanks.csv \\  
    \--confirm-ql 0.05 \--confirm-data ql\_check.csv \--reporting-threshold 0.05  
...
\- python3 compare\_methods.py \-i paired.csv \--margin 10 \--relative \\  
